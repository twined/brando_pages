...
defmodule Mix.Tasks.BrandoPages.InstallTest do
  ...
  test "brando.pages.install" do
    Mix.Tasks.BrandoPages.Install.run([])
    assert_received {:mix_shell, :info, ["\nBrando Pages finished installing."]}

    assert [migration_file] =
      Path.wildcard("priv/repo/migrations/*_create_pages.exs")

    assert_file migration_file, fn file ->
      assert file =~ "defmodule BrandoPages.Repo.Migrations.CreatePages"
      assert file =~ "use Brando.Villain, :migration"
      assert file =~ "villain"
    end

    assert [migration_file] =
      Path.wildcard("priv/repo/migrations/*_create_page_fragments.exs")

    assert_file migration_file, fn file ->
      assert file =~ "defmodule BrandoPages.Repo.Migrations.CreatePageFragments"
      assert file =~ "use Brando.Villain, :migration"
      assert file =~ "villain"
    end
